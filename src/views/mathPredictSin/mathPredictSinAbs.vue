...
export default {
  data() {
    return {
      ...
      lossData: [],
      config: {
        learnRate: 50, // 学习率，太高容易错过极值点，太低学的慢，效率低
  ...
  methods: {
    initTrain() {
      // 初始化用于训练的数据
      let data = genData.genSinAbsData(100, {
        a: this.config.a,
        interval: 10,
        noise: false,
      });
    ...
    initPredict() {
      let xs = Array.from(Array(10000), (v, k) => k);
      let ys = Array.from(Array(xs.length), (v, k) => k === 0 ? 100 : this.config.a * Math.sin(k * (2 * Math.PI / 360)) / (k * (2 * Math.PI / 360)));
